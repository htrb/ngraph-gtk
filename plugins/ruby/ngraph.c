...
check_id(VALUE klass)
{
  ...
  last = ngraph_get_obj_last_id(inst->obj);
  if (inst->id <= last) {
    arg.num = 0;
    r = ngraph_getobj(inst->obj, "oid", inst->id, &arg, &oid);
  ...
  id = ngraph_get_id_by_oid(inst->obj, inst->oid);
  inst->id = id;
...
ngraph_inst_method_move_up(VALUE klass)
{
  ...
  inst->id = ngraph_move_up(inst->obj, inst->id);
...
ngraph_inst_method_move_down(VALUE klass)
{
  ...
  inst->id = ngraph_move_down(inst->obj, inst->id);
...
ngraph_inst_method_move_top(VALUE klass)
{
  ...
  inst->id = ngraph_move_top(inst->obj, inst->id);
...
ngraph_inst_method_move_last(VALUE klass)
{
  ...
  inst->id = ngraph_move_last(inst->obj, inst->id);
...
ngraph_inst_method_exchange(VALUE self, VALUE arg)
{
  ...
  r = ngraph_exchange(inst1->obj, inst1->id, inst2->id);
  if (r < 0) {
    return Qnil;
  }
...
ngraph_inst_method_copy(VALUE self, VALUE arg)
{
  ...
  r = ngraph_copy(inst1->obj, inst1->id, inst2->id);
  if (r < 0) {
    return Qnil;
  }
...
ngraph_inst_method_to_str(VALUE klass)
{
  ...
  name = ngraph_get_obj_name(inst->obj);
...
str2inst_get_ary(VALUE data1)
{
  ...
  name = ngraph_get_obj_name(obj_ids->obj);
...
obj_get(VALUE klass, VALUE id_value, const char *name)
{
  ...
  inst->rcode = id;
  arg.num = 0;
  ngraph_getobj(inst->obj, "oid", inst->id, &arg, &oid);
  inst->oid = oid.i;
...
obj_field_args(VALUE klass, VALUE field, const char *name)
{
  ...
  args = ngraph_get_obj_field_args(nobj, StringValueCStr(field));
  if (args == NULL) {
    args = "";
  } else if (args[0] == '\0') {
    args = "void";
...
obj_field_permission(VALUE klass, VALUE field, const char *name)
{
  ...
  nobj = ngraph_get_object(name);
  perm = ngraph_get_obj_field_permission(nobj, StringValueCStr(field));
...
add_child(VALUE ary, struct objlist *parent, int noinst)
{
  ...
  ocur = ngraph_get_obj_root();
  while (ocur) {
    if (ngraph_get_obj_parent(ocur) == parent) {
      id = ngraph_get_obj_last_id(ocur);
      if (id != -1 || ! noinst) {
	name = ngraph_get_obj_name(ocur);
...
obj_each(VALUE klass, const char *name)
{
  ...
  nobj = ngraph_get_object(name);
  n = ngraph_get_obj_last_id(nobj) + 1;
  if (n < 1) {
    return klass;
  }

  name = ngraph_get_obj_name(nobj);
  ary = rb_ary_new2(n);
  for (i = 0; i < n; i++) {
    id = INT2FIX(i);
...
obj_exchange(VALUE klass, VALUE arg1, VALUE arg2, const char *name)
{
  ...
  r = ngraph_exchange(nobj, id1, id2);
  if (r < 0) {
    return Qnil;
  }
...
obj_current(VALUE klass, const char *name)
{
  ...
  nobj = ngraph_get_object(name);
  id = ngraph_get_obj_current_id(nobj);
  if (id < 0) {
    return Qnil;
  }
...
inst_get_int(VALUE self, const char *field)
{
  ...
  carg.num = 0;
  inst->rcode = ngraph_getobj(inst->obj, field, inst->id, &carg, &num);
  if (inst->rcode < 0) {
    return Qnil;
  }
...
inst_get_str(VALUE self, const char *field)
{
  ...
  carg.num = 0;
  inst->rcode = ngraph_getobj(inst->obj, field, inst->id, &carg, &str);
  if (inst->rcode < 0) {
    return Qnil;
  }
...
inst_get_obj(VALUE self, const char *field)
{
  ...
  obj = ngraph_get_instances_by_str(str.str, &n, &ids);
  if (obj == NULL) {
    return Qnil;
  }
...
inst_put_int(VALUE self, VALUE arg, const char *field)
{
  ...
  num.i = NUM2INT(arg);
  inst->rcode = ngraph_putobj(inst->obj, field, inst->id, &num);
  if (inst->rcode < 0) {
    return Qnil;
  }
...
inst_put_str(VALUE self, VALUE arg, const char *field)
{
  ...
  inst->rcode = ngraph_putobj(inst->obj, field, inst->id, &str);
  if (inst->rcode < 0) {
    return Qnil;
  }
...
inst_put_obj(VALUE self, VALUE arg, const char *field)
{
  ...
  case T_STRING:
    ...
    obj = ngraph_get_instances_by_str(ptr, NULL, &ids);
    if (obj == NULL) {
  ...
  default:
    ...
    name = ngraph_get_obj_name(inst2->obj);
#if 0
  ...
  str.str = ptr;
  inst1->rcode = ngraph_putobj(inst1->obj, field, inst1->id, &str);
  if (inst1->rcode < 0) {
    return Qnil;
  }
...
inst_put_iarray(VALUE self, VALUE arg, const char *field)
{
  ...
  inst->rcode = ngraph_putobj(inst->obj, field, inst->id, &ary);
...
inst_get_iarray(VALUE self, const char *field)
{
  ...
  carg.num = 0;
  inst->rcode = ngraph_getobj(inst->obj, field, inst->id, &carg, &cary);
  if (inst->rcode < 0) {
    return Qnil;
  }
...
add_obj_name_const(VALUE klass, struct objlist *nobj, const char *name)
{
  ...
  if (nobj == NULL) {
    ...
    obj_name = ngraph_get_obj_name(nobj);
...
add_obj_const(VALUE klass, const char *name)
{
  ...
  nobj = ngraph_get_object(name);
  next = ngraph_get_obj_next(nobj);
  child = ngraph_get_obj_child(nobj);
  parent = ngraph_get_obj_parent(nobj);

  str = ngraph_get_obj_version(nobj);
...
ngraph_str2inst(VALUE module, VALUE arg)
{
  ...
  obj_ids.obj = ngraph_get_instances_by_str(str, &obj_ids.num, &obj_ids.ids);
  if (obj_ids.obj == NULL) {
    return Qnil;
  }
